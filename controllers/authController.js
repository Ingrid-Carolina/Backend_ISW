...
`;
  ...
  static async registrarenvivo(req, res) {
    ...
    try {
      ...
      const inserted = await sql`INSERT INTO envivo ${sql(
        urldata
      )} RETURNING id_envivo`;
  ...
  static async obtenerenvivo(req, res) {
    ...
    `;
      ...
      return res
        .status(500)
        .send({ mensaje: "Error al cargar los videos: " + err.message });
    }
  }
  ...
  static async setMostrarAnuncio(req, res) {
    ...
    `;
      ...
      return res
        .status(200)
        .send({ mensaje: "OK", mostrar_anuncio: r[0].mostrar_anuncio });
    } catch (e) {
  ...
  static async obteneridorden(req, res) {
    try {
      const idorden =
        await sql`SELECT idorden FROM ORDENES ORDER BY idorden DESC LIMIT 1`;
      return res.status(200).json(idorden);
    } catch (error) {
      console.error("Error al obtener idorden:", error);
      return res
        .status(500)
        .json({ mensaje: "Error al obtener id de la orden" });
    }
  }
  ...
  static async obtenerTextosAliados(req, res) {
    try {
      ...
      console.log(
        `✅ ${textos.length} textos de aliados cargados exitosamente`
      );
  ...
  static async actualizarTextoAliados(req, res) {
    try {
      ...
      console.log("🔄 Solicitud de actualización de texto de aliados:", {
        clave,
        valor,
      });
  ...
  static async obtenerTextosHistoria(req, res) {
    try {
      ...
      console.log(
        `✅ ${textos.length} textos de historia cargados exitosamente`
      );
  ...
  static async actualizarTextoHistoria(req, res) {
    try {
      ...
      console.log("📝 Solicitud de actualización de texto de historia:", {
        clave,
        valor,
      });
  ...
  static async actualizarMultiplesTextosHistoria(req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        mensaje:
          "Error interno del servidor al procesar los textos de historia",
        error:
          process.env.NODE_ENV === "development" ? error.message : undefined,
      });
    }
